...
void turtleDestroy(Turtle* turtle) {
    if (turtle->shape != NULL) sfConvexShape_destroy(turtle->shape);
    if (turtle->lines != NULL) sfVertexArray_destroy(turtle->lines);
...
void turtleForward(Turtle* turtle, float dist) {
    ...
    if (stepLength <= 0.0f) stepLength = 1.0f;
...
void turtleUpdateDisplay(Turtle* turtle) {
    if (!turtleProcessEvents(turtle)) return;
...
void turtleAppRun(TurtleApp* app) {
    ...
    while (sfRenderWindow_isOpen(app->window)) {
        ...
        while (sfRenderWindow_pollEvent(app->window, &event)) {
            if (event.type == sfEvtClosed) sfRenderWindow_close(app->window);
        }
...
void turtleAppDestroy(TurtleApp* app) {
    ...
    if (app->window != NULL) sfRenderWindow_destroy(app->window);
